...
export function HostsTable({ hosts, select }: { hosts: Host[]; select(id: string): void }) {
  ...
  return <Table aria-label={t("监控实例列表", "Monitoring instance list")}><thead><tr>
    <th scope="col">{t("实例名称", "Instance name")}</th><th scope="col">{t("状态", "Status")}</th><th scope="col">{t("操作系统/架构", "Operating system / architecture")}</th>
    ...
    <th scope="row"><a className="sarmg-instance-link" aria-label={t("选择实例 {0}", "Select instance {0}", [host.name])} href={`#details/${host.id}`} onClick={() => select(host.id)}>{host.name}</a></th>
