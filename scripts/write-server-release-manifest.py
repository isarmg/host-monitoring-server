...
SCHEMA_APPLICATION_VERSION = "0.9.16"
...
def main() -> None:
    if len(sys.argv) != 2:
        fail("usage: write-server-release-manifest.py /absolute/releases/0.9.16")
    root = Path(sys.argv[1])
    if not root.is_absolute() or root.name != VERSION or root.parent.name != "releases":
        fail("root must be an absolute releases/0.9.16 directory")
