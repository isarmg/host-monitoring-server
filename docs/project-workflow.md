...
Linux、机器是 x86_64。正式二进制要求 `--root` 是规范绝对的 `.../releases/0.9.27` 且当前 executable
就是该树的 `bin/host-monitoring-server`；systemd 提供的标准部署根固定为
`/opt/isarmg/host-monitoring/releases/0.9.27`。随后验证 manifest、完整源码 revision、target、API、
...
build.rs 拒绝非 x86_64-unknown-linux-gnu
  ...
  -> 干净且 annotated v0.9.27 tag == HEAD
  -> npm ci + Web build
