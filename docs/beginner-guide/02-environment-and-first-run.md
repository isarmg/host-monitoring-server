...
这个 Node 版本同时满足 Foundation Server 0.9.1 包的 engine 合同；React `19.2.8`、Vite `7.3.6` 与 TypeScript
...
`host-monitoring-client` 仓库及其 CI 中验证。
...
切换到独立 `host-monitoring-client` 仓库，从其 `config/host-monitor.json.example` 创建仅用于临时环境的
