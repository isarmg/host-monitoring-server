...
| HOST-035 | Server 共享原语固定 Foundation `=0.8.9` + 完整 revision `2908f7598571a47c1469bfa08a08c4776cc93049`；八个 Web 包固定同版 Release URL + lock integrity；Client 使用独立上游 | 根 `Cargo.toml`、`web/{package.json,package-lock.json}`、Foundation 门禁 | 保障 | 高 | 认证、构建及 SQLite 基线漂移；sibling/path 会破坏独立 checkout | Foundation 合同、完整 rev/八个 URL/integrity、Router→Client、Web clean build、SQLite reopen |
