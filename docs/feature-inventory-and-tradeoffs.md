...
| HOST-035 | Server 共享原语固定 Foundation `=0.8.3` + 完整 revision `937f543e38ae34c7520672e465c4f9d3f8010e81`；八个 Web 包固定同版 Release URL + lock integrity；Client 使用独立上游 | 根 `Cargo.toml`、`web/{package.json,package-lock.json}`、Foundation 门禁 | 保障 | 高 | 认证、构建及 SQLite 基线漂移；sibling/path 会破坏独立 checkout | Foundation 合同、完整 rev/八个 URL/integrity、Router→Client、Web clean build、SQLite reopen |
...
- Server 只接受 `0.9.19` 配置、数据库与发行身份；不包含转换器或平行 alias。Client 配置格式版本独立冻结为 `0.9.4`。
