...
报告协议为 schema **3**。服务端校验并只接受该协议。扩展位于 `system.hardware`：CPU 硬件详情、物理网卡与接口属性、硬件传感器、磁盘健康。客户端保留系统基础指标，不采集进程。
...
网络接口的流量计数与 IP/MAC/链路属性按唯一接口名合并显示；接口属性不代表物理网卡。Client 独立上报已确认的物理网卡，管理页将其列在“网络硬件”中；没有识别结果时上报空列表。物理磁盘的 SMART 健康信息与文件系统挂载卷不是一一对应关系，因此分别展示。
