...
主机在线状态按 Server 最近接收报告的时间及报告周期估算；详情中的最新指标按客户端采集时间排序，因此补传旧报告不会覆盖更新的读数。详情同时显示采集时间和接收时间；当在线主机的指标采集时间明显早于接收时间时，页面提示可能正在补传或客户端时钟发生变化。采集时间明显晚于接收时间时，页面提示客户端时钟可能偏快，后续指标可能暂时不更新。历史趋势仅连接相邻采样桶，空缺时显示断线，孤立样本显示为点。
