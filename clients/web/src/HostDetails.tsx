...
export function HostDetails({ hostId, changed }: { hostId: string; changed(): void }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (!stopped && !paused && !document.hidden) timer = window.setTimeout(refresh, 2_000);
      }
    }
    const visible = () => { if (!document.hidden && !paused) { if (timer !== undefined) clearTimeout(timer); void refresh(); } };
  ...
  }, [client, hostId, paused, dirty]);

  useEffect(() => {
    const controller = new AbortController();
    const from = new Date(Date.now() - historyHours * 60 * 60 * 1000).toISOString();
    void client.request(`/api/v2/monitoring/hosts/${hostId}/history?from=${encodeURIComponent(from)}&resolution=auto&max_points=720`, isHistorySeriesResponse, { signal: controller.signal })
      .then(value => { if (!controller.signal.aborted && value.host_id === hostId) setHistory(value); })
      .catch(() => { /* latest detail remains useful when history is temporarily unavailable */ });
    return () => controller.abort();
  }, [client, hostId, historyHours, updatedAt && Math.floor(updatedAt.getTime() / (historyHours <= 1 ? 5_000 : 30_000))]);
  ...
  return <div className="sarmg-content-stack">
    ...
    <HistoryChart response={history} hours={historyHours} changeHours={setHistoryHours} />
...
function HistoryChart({ response, hours, changeHours }: { response: HistorySeriesResponse | null; hours: number; changeHours(value: number): void }) {
  const series = (response?.points ?? []).filter(point => point.cpu_usage_percent.avg !== null || point.memory_usage_percent.avg !== null);
  const path = (key: "cpu_usage_percent" | "memory_usage_percent") => series.map((point, index) => {
    const value = point[key].avg;
    const x = series.length <= 1 ? 0 : index * 100 / (series.length - 1);
    return value === null ? null : `${x},${100 - value}`;
  ...
  return <section className="sarmg-content-panel" aria-labelledby="history-heading"><h2 id="history-heading">{t("历史趋势", "History trends")}</h2>
    ...
    <p>{response ? t("每点 {0} 秒，来源：{1}；曲线为样本均值，缺失值不补零。", "{0} seconds per point, source: {1}; lines show sample averages and missing values are not filled.", [String(response.step_seconds), response.source]) : t("正在读取历史趋势…", "Loading history trends…")}</p>
    {series.length === 0 ? <p>{t("所选范围没有历史样本", "No historical samples in this range")}</p> : <svg viewBox="0 0 100 100" role="img" aria-label={t("CPU 与内存使用率历史图", "CPU and memory usage history chart")} preserveAspectRatio="none" style={{ width: "100%", height: "16rem" }}>
      <polyline points={path("cpu_usage_percent")} fill="none" stroke="currentColor" vectorEffect="non-scaling-stroke" />
      <polyline points={path("memory_usage_percent")} fill="none" stroke="#7a5aa6" vectorEffect="non-scaling-stroke" />
    </svg>}
  </section>;
}
